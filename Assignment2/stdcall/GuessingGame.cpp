...
void __attribute__( (__stdcall__)) Dice::roll(){
    random_device rd;
...
int __attribute__( (__stdcall__)) Dice::getRolled(){
    return rolled;
}

void __attribute__( (__stdcall__)) Player::guess(int a){
    guesses.push_back(a);
}

int __attribute__( (__stdcall__)) Player::getLastGuess(){
    if (guesses.size() == 0){
        return -1;
    } else {
...
int __attribute__( (__stdcall__)) Player::getNumGuesses(){
    return guesses.size();    
}

void __attribute__( (__stdcall__)) Player::addWin(){
    score++;
}

int __attribute__( (__stdcall__)) Player::getNumWins(){
    return score;
}
